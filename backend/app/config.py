...
class Settings(BaseSettings):
    ...
    audio_storage_dir: Path = Field(
        default=AUDIO_STORAGE_DIR, validation_alias="SPEAKLY_AUDIO_DIR"
    )
    ...
    ollama_model_summary: str = Field(
        default="llama3", validation_alias="OLLAMA_MODEL_SUMMARY"
    )
    ollama_model_todo: str = Field(
        default="llama3", validation_alias="OLLAMA_MODEL_TODO"
    )
    ...
    ticktick_client_id: str | None = Field(
        default=None, validation_alias="TICKTICK_CLIENT_ID"
    )
    ticktick_client_secret: str | None = Field(
        default=None, validation_alias="TICKTICK_CLIENT_SECRET"
    )
    ...
    ticktick_base_url: str = Field(
        default="https://api.ticktick.com/open/v1", validation_alias="TICKTICK_BASE_URL"
    )
    frontend_base_url: str = Field(
        default="http://localhost:5173", validation_alias="SPEAKLY_FRONTEND_URL"
    )

    # Feature Flags (for monetization/rollout control)
    feature_auto_tagging: bool = Field(
        default=True, validation_alias="SPEAKLY_FEATURE_AUTO_TAGGING"
    )
    feature_custom_tags: bool = Field(
        default=True, validation_alias="SPEAKLY_FEATURE_CUSTOM_TAGS"
    )
    feature_advanced_search: bool = Field(
        default=True, validation_alias="SPEAKLY_FEATURE_ADVANCED_SEARCH"
    )
    feature_report_generation: bool = Field(
        default=False, validation_alias="SPEAKLY_FEATURE_REPORT_GENERATION"
    )
    feature_command_palette: bool = Field(
        default=True, validation_alias="SPEAKLY_FEATURE_COMMAND_PALETTE"
    )

    # Tagging Configuration
    max_tags_per_session: int = Field(
        default=5, validation_alias="SPEAKLY_MAX_TAGS_PER_SESSION"
    )

    # Usage Limits (for free tier)
    free_recordings_per_month: int = Field(
        default=10, validation_alias="SPEAKLY_FREE_RECORDINGS_PER_MONTH"
    )
    free_tasks_per_session: int = Field(
        default=3, validation_alias="SPEAKLY_FREE_TASKS_PER_SESSION"
    )
    free_tags_per_session: int = Field(
        default=3, validation_alias="SPEAKLY_FREE_TAGS_PER_SESSION"
    )
    ...
    def pj_voice_tag_set(self) -> set[str]:
        return {
            tag.strip().lower() for tag in self.pj_voice_tags.split(",") if tag.strip()
        }
