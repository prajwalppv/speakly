        ...
        for integration in self._integrations:
            try:
                ...
                self._update_todo_sync_status(
                    todo, integration.name, result, db
                )
            ...
            try:
                ...
                for integration in self._integrations.values():
                    ...
                    if hasattr(integration, "process_task_update"):
                        result = await integration.process_task_update(update, user_id, db)
                        
                        if result.get("success"):
                            stats["updated"] += 1
                            logger.info(
                                f"Updated task via {integration.name}",
                                extra={"integration": integration.name, "result": result}
                            )
    ...
    async def delete_from_ticktick(self, task_id: str, project_id: str) -> None:
        ...
        with SessionLocal() as db:
            ...
            try:
                ...
                logger.info(f"Deleted task {task_id} from project {project_id} in TickTick")
            except Exception as e:
...
async def _async_task_sync(session_id: int, task_updates: list[dict] | None = None) -> None:
