...
from ..models import Todo, Transcription
...
class GroqProvider(LlmProvider):
    ...
    def __init__(self, settings) -> None:
        ...
        self._model = (
            getattr(settings, "groq_model", "") or "gpt-4o-mini"
        ).strip() or "gpt-4o-mini"
        self._client = None
        base_url = (
            getattr(settings, "groq_api_base_url", "https://api.groq.com")
    ...
    def _get_client(self):
        ...
        if self._client is None:
            ...
            if Groq is None:
                raise LlmError(
                    "Groq SDK is not installed. Install the 'groq' package to enable Groq support."
                )
    ...
    def generate(self, prompt: str, task: LlmTask) -> str:
        ...
        @retry(
            retry=retry_if_exception_type(
                (GroqRateLimitError, httpx.TimeoutException, httpx.NetworkError)
            ),
...
class OllamaProvider(LlmProvider):
    ...
    def __init__(self, settings) -> None:
        ...
        self._base_url = base_url.rstrip("/")
        summary_model = (
            getattr(settings, "ollama_model_summary", "") or "llama3"
        ).strip()
        self._summary_model = summary_model or "llama3"
        todo_model = (
            getattr(settings, "ollama_model_todo", "") or self._summary_model
        ).strip()
...
class LlmService:
    ...
    def __init__(self, provider_name: str | None = None) -> None:
        raw_requested = (
            provider_name or getattr(settings, "llm_provider", "auto") or "auto"
        )
...
async def schedule_summary_and_todos(session_id: int, transcription_id: int) -> None:
    ...
    async with semaphore:
        ...
        await loop.run_in_executor(
            None, _run_summary_and_todos, session_id, transcription_id
        )
        logger.info(
...
def _generate_title(
    service: LlmService, session: SessionModel, transcript: str, db
) -> bool:
...
def _generate_summary(
    service: LlmService, summary_run: LlmRun, transcript: str, session: SessionModel
) -> bool:
...
def _extract_and_create_todos(
    ...
    """
    extraction_result: dict[str, Any] = {"new_tasks": [], "task_updates": []}
    try:
...
def _run_summary_and_todos(session_id: int, transcription_id: int) -> None:
    ...
    with SessionLocal() as db:
        ...
        todos_success, created, task_updates = _extract_and_create_todos(
            service, todo_run, transcript, session, db
        )
        ...
        if session.processing_stages:
            ...
            if created > 0 or task_updates:
                stages["syncing_tasks"] = {"status": "pending", "timestamp": None}
            else:
                # No tasks to sync
        ...
        if settings.feature_auto_tagging:
            try:
                ...
                logger.error(
                    f"Failed to extract tags for session {session.id}: {e}",
                    exc_info=True,
                )
        ...
        if failed_stages:
            session.last_error = (
                f"Partial processing failure in stages: {', '.join(failed_stages)}"
            )
