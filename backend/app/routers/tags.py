...
from ..models import Tag, SessionTag, Session as SessionModel
...
async def list_tags(
    ...
    query = db.query(
        Tag,
        func.count(SessionTag.id).label('usage_count')
    ).outerjoin(SessionTag).group_by(Tag.id)
...
async def create_tag(
    ...
    tag = Tag(
        ...
        color=tag_data.color or tagging_service.assign_color(tag_data.category or "custom"),
...
@router.post("/sessions/{session_id}/tags/{tag_id}", status_code=status.HTTP_204_NO_CONTENT)
...
async def remove_tag_from_session(
    ...
    deleted = db.query(SessionTag).filter(
        SessionTag.session_id == session_id,
        SessionTag.tag_id == tag_id
    ).delete()
