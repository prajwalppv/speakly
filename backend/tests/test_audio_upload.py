...
class TestAudioUploadAPI:
    ...
    def test_upload_audio_stores_file(
        self, mock_get_client, client, test_db, test_audio_file
    ):
    ...
    @patch("app.routers.audio.get_elevenlabs_client")
    def test_upload_multiple_files_creates_multiple_sessions(
        self, mock_get_client, client, test_db, test_audio_file
    ):
        ...
        # Upload first file
        response1 = client.post("/api/audio", files={"audio": test_audio_file})
        session_id1 = response1.json()["session_id"]

        # Upload second file
        response2 = client.post("/api/audio", files={"audio": test_audio_file})
...
class TestAudioUploadEdgeCases:
    ...
    def test_upload_large_filename(self, mock_get_client, client, test_db):
        ...
        response = client.post("/api/audio", files={"audio": audio_file})
    ...
    def test_upload_sets_pending_status(self, client, test_db):
        ...
        transcription = (
            test_db.query(Transcription).filter_by(session_id=session.id).first()
        )
        assert transcription.status in ["pending", "submitted"]
...
class TestAudioUploadErrorHandling:
    ...
    def test_upload_handles_elevenlabs_not_configured(self, client, test_db):
        ...
        transcription = (
            test_db.query(Transcription).filter_by(id=data["transcription_id"]).first()
        )
...
class TestBulkAudioUpload:
    ...
    def test_bulk_upload_multiple_files(self, mock_get_client, client, test_db):
        ...
        files = [
            (
                "files",
                ("file1.mp3", b"\xff\xfb\x90\x00" + b"\x00" * 1024, "audio/mpeg"),
            ),
            (
                "files",
                ("file2.mp3", b"\xff\xfb\x90\x00" + b"\x00" * 1024, "audio/mpeg"),
            ),
            (
                "files",
                ("file3.mp3", b"\xff\xfb\x90\x00" + b"\x00" * 1024, "audio/mpeg"),
            ),
        ]
    ...
    def test_bulk_upload_single_file(self, mock_get_client, client, test_db):
        ...
        files = [
            (
                "files",
                ("single.mp3", b"\xff\xfb\x90\x00" + b"\x00" * 1024, "audio/mpeg"),
            ),
        ]
