...
from app.models import Session, Tag, Transcription, User
...
class TestBasicModels:
    ...
    def test_create_session(self, test_db):
        ...
        session = Session(user_id=1, audio_path="/path/to/audio.mp3", status="pending")
        test_db.add(session)
        test_db.commit()
    ...
    def test_create_transcription(self, test_db):
        ...
        transcription = Transcription(
            session_id=session.id, provider="elevenlabs", text="Test transcript"
        )
    ...
    def test_create_tag(self, test_db):
        """Test creating a tag."""
        tag = Tag(name="meeting", category="event", color="#3b82f6")
        test_db.add(tag)
        test_db.commit()
    ...
    def test_transcription_metadata(self, test_db):
        ...
        transcription = Transcription(session_id=session.id, metadata_payload=metadata)
        test_db.add(transcription)
        test_db.commit()
