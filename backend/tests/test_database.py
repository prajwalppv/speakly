...
class TestDatabaseRelationships:
    ...
    def test_cascade_delete_behavior(self, test_db):
        ...
        remaining_trans = (
            test_db.query(Transcription).filter_by(session_id=session_id).all()
        )
...
class TestDatabaseConstraints:
    ...
    def test_unique_user_name_constraint(self, test_db):
        ...
        user1 = User(
            name="unique_test", email="test1@test.com", clerk_user_id="clerk_123"
        )
        test_db.add(user1)
        test_db.commit()
        ...
        user2 = User(
            name="different_name", email="test2@test.com", clerk_user_id="clerk_123"
        )
        test_db.add(user2)
...
class TestDatabaseQueries:
    ...
    def test_filter_query(self, test_db):
        ...
        completed = test_db.query(Session).filter(Session.status == "completed").all()
    ...
    def test_order_by_query(self, test_db):
        ...
        # Query with order
        sessions = test_db.query(Session).order_by(Session.created_at.desc()).all()
    ...
    def test_join_query(self, test_db):
        ...
        # Query with join
        results = (
            test_db.query(Session).join(User).filter(User.name == "join_test").all()
        )
...
class TestDatabaseMetadata:
    ...
    def test_json_field_storage(self, test_db):
        ...
        transcription = Transcription(
            session_id=session.id, metadata_payload=metadata, status="completed"
        )
        test_db.add(transcription)
        test_db.commit()
