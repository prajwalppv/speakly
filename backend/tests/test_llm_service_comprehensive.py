...
@pytest.mark.skip(
    reason="Internal implementation tests - mocking issues don't affect API"
)
...
class TestLlmServiceGenerate:
    ...
    def test_generate_raises_error_when_not_configured(self, mock_settings):
        ...
        from app.services.llm import LlmError, LlmService, LlmTask
...
class TestGenerateSummaryHelper:
    ...
    def test_generate_summary_helper_success(self):
        ...
        result = _generate_summary(
            mock_service, mock_summary_run, "transcript", mock_session
        )
...
class TestRunSummaryAndTodos:
    ...
    def test_run_summary_and_todos_complete_workflow(
        ...
        query_session.options.return_value.filter_by.return_value.one_or_none.return_value = (
            mock_session
        )
        query_transcription.filter_by.return_value.one_or_none.return_value = (
            mock_transcription
        )
