...
class TestBulkUploadOrdering:
    ...
    def test_bulk_upload_maintains_context_order(self, client, test_db):
        ...
        files = [
            ("files", ("meeting_20240101_143000.wav", BytesIO(b"afternoon"), "audio/wav")),  # Afternoon
            ("files", ("meeting_20240101_090000.wav", BytesIO(b"morning"), "audio/wav")),    # Morning
        ]
    ...
    def test_bulk_upload_logs_ordering(self, client, test_db, caplog):
        ...
        assert any("timestamp order" in record.message.lower() for record in caplog.records)
...
class TestEdgeCases:
    ...
    def test_different_timestamp_formats_mixed(self):
        ...
        files = [
            UploadFile(filename="file_20240103100000.wav", file=BytesIO(b"3")),  # Pattern 3
            UploadFile(filename="rec_2024-01-01_10-00-00.wav", file=BytesIO(b"1")),  # Pattern 2
            UploadFile(filename="voice_20240102_100000.wav", file=BytesIO(b"2")),  # Pattern 1
        ]
        
        sorted_files = _sort_files_by_timestamp(files)
        
        # Should correctly sort regardless of format
        assert "2024-01-01" in sorted_files[0].filename or "20240101" in sorted_files[0].filename
        assert "20240102" in sorted_files[1].filename
