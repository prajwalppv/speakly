...
class TestSyncTask:
    ...
    async def test_sync_task_disabled(self, mock_settings):
        ...
        from app.integrations.ticktick_integration import TickTickIntegration, IntegrationError
    ...
    @patch('app.integrations.ticktick_integration.TickTickClient')
    @patch('app.integrations.ticktick_integration.settings')
    async def test_sync_task_with_transcript_snippet(self, mock_settings, mock_client_class):
        ...
        mock_client = Mock()
        mock_client.get_or_create_speakly_project = AsyncMock(return_value="proj-1")
        mock_client.create_task = AsyncMock(return_value={"id": "t1", "projectId": "p1"})
        mock_client_class.return_value = mock_client
...
class TestGetOrCreateProject:
    ...
    @patch('app.integrations.ticktick_integration.TickTickClient')
    @patch('app.integrations.ticktick_integration.settings')
    async def test_get_or_create_project_success(self, mock_settings, mock_client_class):
...
class TestGetProjects:
    ...
    async def test_get_projects_success(self, mock_settings, mock_client_class):
        ...
        mock_settings.ticktick_enabled = True
        mock_client = Mock()
        mock_client.get_projects = AsyncMock(return_value=[{"id": "p1", "name": "Test"}])
        mock_client_class.return_value = mock_client
...
class TestSearchTasks:
    ...
    async def test_search_tasks_no_matches(self, mock_settings, mock_client_class):
        ...
        integration = TickTickIntegration()
        results = await integration.search_tasks("completely different query", 1, Mock())
...
class TestUpdateTaskStatus:
    ...
    @patch('app.integrations.ticktick_integration.TickTickClient')
    @patch('app.integrations.ticktick_integration.settings')
    async def test_update_task_status_in_progress(self, mock_settings, mock_client_class):
...
class TestProcessTaskUpdate:
    ...
    async def test_process_task_update_success(self, mock_settings):
        ...
        integration = TickTickIntegration()
        integration.search_tasks = AsyncMock(return_value=[
            {"id": "t1", "title": "Matching task", "match_score": 95}
        ])
