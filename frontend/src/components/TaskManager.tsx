...
import { Plus, X, Edit2, Save, Trash2, RefreshCw, Check, Clock, AlertCircle, Link as LinkIcon } from 'lucide-react';
...
const TaskManager: React.FC<TaskManagerProps> = ({ sessionId, todos, onUpdate }) => {
  const [isAdding, setIsAdding] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [newTask, setNewTask] = useState({ title: '', due_hint: '', source_excerpt: '' });
  ...
  const getSyncStatusBadge = (syncStatus: string, taskId: string | null) => {
    const configs: Record<string, { icon: any; bg: string; text: string; label: string }> = {
      synced: { icon: Check, bg: 'bg-green/20', text: 'text-green', label: 'Synced' },
      pending: { icon: Clock, bg: 'bg-gold/20', text: 'text-gold', label: 'Pending' },
      error: { icon: AlertCircle, bg: 'bg-red-500/20', text: 'text-red-500', label: 'Error' },
    };
    ...
    return (
      <span className={cn('flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium', config.bg, config.text)}>
        <Icon size={12} />
        {config.label}
        {taskId && (
        ...
        {isAdding && (
          ...
          >
            ...
            <input
              ...
              onChange={(e) => setNewTask({ ...newTask, due_hint: e.target.value })}
            />
            <textarea
              ...
              onChange={(e) => setNewTask({ ...newTask, source_excerpt: e.target.value })}
              rows={2}
            />
            <div className="flex gap-2 justify-end">
              ...
              <motion.button
                ...
                whileHover={newTask.title.trim() && !isLoading ? { scale: 1.05 } : {}}
                whileTap={newTask.title.trim() && !isLoading ? { scale: 0.95 } : {}}
              >
      ...
      <div className="space-y-2">
        {todos.length === 0 && !isAdding && (
          <div className="bg-black/20 border border-bone-dim/20 rounded-lg p-6 text-center space-y-2">
            ...
            <p className="text-bone-dim text-xs">Add tasks manually or they'll be extracted from your audio</p>
          </div>
        )}
            ...
            {editingId === todo.id ? (
              <div className="space-y-3">
                ...
                <input
                  ...
                  onChange={(e) => setEditTask({ ...editTask, due_hint: e.target.value })}
                  onClick={(e) => e.stopPropagation()}
                  onFocus={(e) => e.stopPropagation()}
                />
                <div className="flex gap-2 justify-end">
              ...
              <>
                <div className="space-y-2 flex-1">
                  <div className="flex items-start justify-between gap-3">
                    <h5 className="text-sm font-medium text-bone flex-1">{todo.title}</h5>
                    {getSyncStatusBadge(todo.ticktick_sync_status, todo.ticktick_task_id)}
                  </div>
                  ...
                  {todo.confidence !== null && (
                    <div className={cn(
                      ...
                      todo.confidence >= 0.9 ? 'text-green' : 
                      todo.confidence >= 0.8 ? 'text-gold' : 
                      'text-bone-dim'
                    )}>
